...
```
Because what we are talking about is the base or foundation needed to know. We can not know how to fix something that we don't know what it is.
```
...
```
CPU the brains of the hardware
```

**Operating system layer — name an OS (e.g., Windows, Linux, or macOS):** Windows

```
Windows is a common OS used by many companies and everyday user due to the many additional programs and applications synced with Windows
```

**Software layer — one example application (e.g., a web browser or word processor):** Wod Processor

```
Microsoft Word a common application
```
...
**Explanation (minimum 3 sentences):** Software-OS-Hardware

```
When we open a program or file it knocks on the OS door to say I have a request or need and then the OS knocks on the Hardware door and wakes it up to say it needs something in order to help the Software. Then the Hardware will say yay or nay to having resources to give to the OS to give to the software.
```
...
```
The OS is the middle layer on my diagram which is the one that would stop working. I believe that the Software would crash and the hardware would freeze because in order to do anything each needs the other.
```
...
```
I use Google alot. I search things in the search bar often so then the OS starts looking at what Google needs to search then it speaks to the hardware to to find out if it has the resources or ability to handle a search. If the hardware layer fails I assume that the Software would freeze or crash with error messages.
```
...
```
I feel like a CyberSecurity professional needs to understand all three layers-hardware, os, software because the virtual and physical environment requires attackers and defenders to constantly adapt. Because of that we need to know the playing field that they may use (entry points).
```
...
```
The cybersecurity landscape has 4 Moving Parts that continues to grow and evolve with the ever changing and growing amount of devices and systems that are getting attacked and need to be defended. Someone starting the course needs to understand that because as we start to learn more we will always have that as 1 of the constants in a world of variables.
```
...
```
We identified the Foundry District. Part A connected to Part B because it helped us understand the foundation of what attackers and defenders use against each other as well as everyday Users- and how those layers talk to each other to make systems and tools and devices keep things moving.
```

**3. Of the three layers (hardware, OS, software), which one do you think is hardest to secure, and why?**

```
I think software is hardest because there are always updates which can have hackers plant stuff in them, there can be doppleganger versions that are made to seem more appealing to unknown users and then they get scammed, they can be planted on all types of devices so sometimes you can have a messed up program because the device has been hacked and now that program is too... just seems like a lot of entry points to get attacked on.
```
